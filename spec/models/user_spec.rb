...
RSpec.describe User, type: :model do
  describe 'ユーザ登録のテスト' do
    ...
    it 'nameが入力文字列が60文字を超える場合は登録ができない' do
      user = build(:user, name: SecureRandom.alphanumeric(60) )
      expect(user).to be_valid
      user = build(:user, name: SecureRandom.alphanumeric(61) )
      expect(user).not_to be_valid
    end
    ...
    it 'emailが正しいルールで書かれてない場合は登録ができない' do
      ...
      expect(user).not_to be_valid
      user = build(:user, email: "test@examplecom")
      expect(user).not_to be_valid
      user = build(:user, email: "テスト@example.com")
      expect(user).not_to be_valid
    end
